...
import { UserCircle, UserPlus, ChatTeardropText, DotsThree, Notepad, Check, Prohibit, UserMinus, UserCheck, X } from '@phosphor-icons/react';
...
const UserProfileModal = ({ user, isOpen, setIsOpen }) => {
    ...
    const pendingRequest = useMemo(() => requests.find(req =>
    ...
    const member = useMemo(() => guildId ? (guildsStore.guildMembers[guildId] || []).find(m => m.user_id === user?.id) : null, [guildId, guildsStore.guildMembers, user?.id]);
    const roles = member?.roles || user?.roles || [];
    const sortedRoles = useMemo(() => [...roles].sort((a, b) => (b.position || 0) - (a.position || 0)), [roles]);
    ...
    return (
        <Dialog
            ...
            noPadding
        >
            <div className="overflow-hidden rounded-xl bg-[#111214] shadow-2xl border border-white/5 w-full max-w-xl">
                {/* Banner */}
                <div
                    className={cn(
                        "h-[120px] w-full",
                        !user.banner_color && "bg-primary"
                    )}
                    style={{
                ...
                <div className="relative px-4 pb-4">
                    ...
                    {/* Actions Corner */}
                    <div className="flex justify-end pt-3 gap-2 h-14">
                        {!isOwner && (
                            <>
                                {isFriend ? (
                                    <button
                                ...
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <button title="More" className="flex size-9 items-center justify-center rounded bg-[#2b2d31] text-gray-300 transition hover:bg-[#35373c] hover:text-white">
                    ...
                    <div className="mt-4 space-y-5 px-1">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-[15px] text-gray-200 leading-normal">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {sortedRoles.map(role => (
                                        <span key={role.id} className="flex items-center gap-1.5 rounded bg-[#2b2d31] px-2 py-1 text-[11px] font-bold text-gray-200">
                                            <div
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <textarea
                                    ...
                                    className="w-full bg-transparent p-1 text-[13px] text-gray-200 placeholder:text-gray-500 focus:outline-none resize-none min-h-[40px] hover:bg-white/5 rounded transition-colors"
                                    rows={2}
                                />
                            </div>
                        </div>
